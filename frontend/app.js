...
/* Reveal the password while it is being typed.
   A password field that cannot be read is where a wrong keyboard layout, a
   stray caps lock or a mistyped handover password go unnoticed — and these
   passwords are generated by us and read off a message, not remembered. The
   field goes back to hidden on sign-out, so a revealed one is never left on a
   shared screen. */
function setPasswordShown(shown) {
  ...
  // Held as a key, not as a finished string, so a language switch while the
  // password is revealed re-says the label instead of stranding the old one.
  button.dataset.tLabel = shown ? "pw_hide" : "pw_show";
  const label = t(button.dataset.tLabel);
...
function togglePassword() {
  const field = setPasswordShown(document.getElementById("password").type === "password");
  field.focus();
  // Typing continues where it left off rather than at the start of the box.
  const end = field.value.length;
  try { field.setSelectionRange(end, end); } catch (e) { /* not every type allows it */ }
}

const hidePassword = () => setPasswordShown(false);
...
/* ---------------- admin ---------------- */
// The accounts as last listed, so the manage dialog opens on what is on screen
// rather than re-fetching one account to fill in a form.
const customerById = {};

async function loadCustomers() {
  // The plans come along so the table can name the one each account is on. The
  // account stores a slug, not a name: a plan renamed for the public page must
  // not rename itself on the accounts that were sold under it.
  const [customers, jobs, list] = await Promise.all([
    api("/api/admin/customers"), api("/api/admin/jobs?limit=200"), api("/api/admin/plans"),
  ]);
  plans = list;
  customers.forEach((customer) => { customerById[customer.id] = customer; });
  ...
  document.getElementById("custTable").innerHTML = customers.length ? `
    <table><thead><tr>
      <th>${esc(t("th_name"))}</th><th>${esc(t("th_mail"))}</th><th>${esc(t("th_plan"))}</th>
      <th>${esc(t("th_status"))}</th><th>${esc(t("th_used"))}</th><th></th></tr></thead>
    <tbody>${customers.map((customer) => {
      ...
      return `<tr>
        ...
        <td data-l="${esc(t("th_plan"))}">${planCell(customer)}</td>
        <td data-l="${esc(t("th_status"))}">${pill}</td>
        <td class="mono" data-l="${esc(t("th_used"))}">${customer.used_this_month} / ${customer.monthly_quota}</td>
        <td class="act"><button class="btn btn-s" onclick="manage('${customer.id}')">${
          esc(t("manage"))}</button></td></tr>`;
    }).join("")}</tbody></table>` : `<div class="empty">${esc(t("nocust"))}</div>`;
}

/* The plan an account is on, and whether it has been switched on.
   Two facts, shown together, because an account on a plan nobody activated
   looks identical to one on no plan at all until you say so. */
function planCell(customer) {
  if (!customer.plan_slug) return `<span class="pill p-idle">${esc(t("pl_none"))}</span>`;
  const plan = plans.find((entry) => entry.slug === customer.plan_slug);
  const name = plan ? planName(plan) : customer.plan_slug;
  const state = customer.plan_active
    ? `<span class="pill p-ok">${esc(t("pl_on"))}</span>`
    : `<span class="pill p-flag">${esc(t("pl_off"))}</span>`;
...
function openNewCustomer() {
  modal(`<div class="card-b" style="display:flex;flex-direction:column;gap:14px">
    ...
    <div class="field"><label>${esc(t("m_plan"))}</label><select id="nPlan">
      <option value="">${esc(t("pl_none"))}</option>
      ${plans.filter((plan) => plan.active).map((plan) =>
        `<option value="${esc(plan.slug)}">${esc(planName(plan))}</option>`).join("")}
    </select></div>
    ${switchRow("nPlanActive", t("m_plan_on"), false)}
...
async function createCustomer() {
  ...
  try {
    const created = await api("/api/admin/customers", { method: "POST", json: {
      ...
      plan_slug: document.getElementById("nPlan").value,
      plan_active: document.getElementById("nPlanActive").checked,
    }});
...
const switchRow = (id, label, on) =>
  `<label class="rowline" style="cursor:pointer"><span>${esc(label)}</span>
     <input type="checkbox" id="${id}"${on ? " checked" : ""}
            style="width:18px;height:18px"></label>`;

/* Managing one account: the limit, the plan, whether the plan is switched on,
   and whether the account itself is. One dialog and one save, because those
   four are decided in a single conversation with the customer — an admin who
   has just raised a limit is the same admin activating the plan that was paid
   for. */
function manage(id) {
  const customer = customerById[id];
  if (!customer) return;
  const options = [`<option value=""${customer.plan_slug ? "" : " selected"}>${
    esc(t("pl_none"))}</option>`].concat(
    plans.filter((plan) => plan.active || plan.slug === customer.plan_slug)
      .map((plan) => `<option value="${esc(plan.slug)}"${
        plan.slug === customer.plan_slug ? " selected" : ""}>${esc(planName(plan))} · ${
        plan.monthly_limit.toLocaleString("en-US")}</option>`),
  ).join("");

  modal(`<div class="card-b" style="display:flex;flex-direction:column;gap:14px">
    <h2 style="font-size:17px">${esc(customer.display_name || customer.email)}</h2>
    <div class="alert a-bad hidden" id="mgErr"></div>

    <div class="field"><label>${esc(t("f_quota"))}</label>
      <input type="number" id="mQuota" min="0" value="${customer.monthly_quota}"></div>
    <p class="note">${esc(t("m_used"))}: ${customer.used_this_month}</p>

    <div class="field"><label>${esc(t("m_plan"))}</label>
      <select id="mPlan" onchange="planPicked()">${options}</select></div>
    ${switchRow("mPlanActive", t("m_plan_on"), customer.plan_active)}
    ${switchRow("mApplyLimit", t("m_apply_limit"), false)}
    ${switchRow("mActive", t("m_account_on"), customer.active)}

    <div style="display:flex;gap:8px">
      <button class="btn btn-p" style="flex:1" onclick="saveCustomer('${id}')">${esc(t("save"))}</button>
      <button class="btn" style="flex:1" onclick="closeModal()">${esc(t("cancel"))}</button>
    </div>
    <button class="btn btn-w" onclick="resetCustomerPassword('${id}')">${esc(t("resetpw"))}</button>
    <p class="note">${esc(t("m_note"))}</p></div>`);
  planPicked();
}

/* Taking the account off every plan cannot leave an activation behind, and
   there is no plan limit to copy across either. The server enforces both; the
   dialog just stops offering what it would refuse. */
function planPicked() {
  const chosen = document.getElementById("mPlan").value;
  const active = document.getElementById("mPlanActive");
  const apply = document.getElementById("mApplyLimit");
  [active, apply].forEach((box) => {
    box.disabled = !chosen;
    if (!chosen) box.checked = false;
    box.closest("label").style.opacity = chosen ? "" : ".5";
  });
}

async function saveCustomer(id) {
  const error = document.getElementById("mgErr");
  clearError(error);
  const slug = document.getElementById("mPlan").value;
  const body = {
    monthly_quota: parseInt(document.getElementById("mQuota").value, 10) || 0,
    plan_slug: slug,
    plan_active: document.getElementById("mPlanActive").checked,
    active: document.getElementById("mActive").checked,
  };
  // Copying the plan's own limit onto the account is asked for explicitly, and
  // never implied by picking a plan: a plan is a price list entry an admin
  // edits freely, and an edit to it must not silently retune a live account.
  if (document.getElementById("mApplyLimit").checked) {
    const plan = plans.find((entry) => entry.slug === slug);
    if (plan) body.monthly_quota = plan.monthly_limit;
  }
  try {
    await api("/api/admin/customers/" + id, { method: "PATCH", json: body });
    closeModal();
    loadCustomers();
  } catch (failure) {
    showError(error, failure);
...
async function loadUsage() {
  ...
  document.getElementById("usageBox").innerHTML = rows.length
    ? rows.map((row) => {
        ...
        const plan = row.plan_slug && !row.plan_active
          ? ` <span class="pill p-flag">${esc(t("pl_off"))}</span>` : "";
        return `<div style="margin-bottom:14px">
          <div class="rowline"><span>${esc(row.name)}${plan}</span>
